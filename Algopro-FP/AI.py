...
# Set recursion limit to handle deep recursion in Alpha-Beta pruning
...
class GomokuAI():
    def __init__(self, depth=3):
        ...
        """
        Initializes the Gomoku AI with a given search depth for Minimax.
        Sets up the board, zobrist hashing, pattern dictionary, and other game state 
        variables.
        """
    ...
    def isValid(self, i, j, state=True):
        """
        Validates a move:
        - state=True checks if the position is empty and within bounds.
        - state=False checks only if the position is within bounds.
        """
    ...
    def setState(self, i, j, state):
        """
        Updates the board at position (i, j) with the given state.
        States:
    ...
    def childNodes(self, bound):
        last_move_bias = lambda x: max(3 - abs(x[0][0] - self.currentI) - abs(x[0][1] - self.currentJ), 1)
        sorted_bound = sorted(bound.items(), key=lambda el: (last_move_bias(el), el[1]), reverse=True)
    ...
    def evaluate(self, new_i, new_j, board_value, turn, bound):
        ...
        center_weight = max(1, 3 - abs(new_i - 7) - abs(new_j - 7))

        # Check for every pattern in patternDict
        for pattern in self.patternDict:
            score = self.patternDict[pattern]*center_weight
        ...
        # Immediate threats and opportunities get additional weight
        if self.isFive(new_i, new_j, turn):
            value_after += 1000000  # Winning move
        elif self.isFive(new_i, new_j, -turn):
            value_after -= 1000000  # Blocking opponent's winning move
